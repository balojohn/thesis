...
class RFIDNode(Node):
    def __init__(self, sensor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"sensor.reader.rfid.{self.sensor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for sensor.reader.rfid
        self.data_publisher = self.create_publisher(
            topic=f"sensor.reader.rfid.{self.sensor_id}",
    ...
    def simulate_rfid(self, name: str):
        """
        Simulate dynamic behavior for 'RFID' actor.
        """
        t = time.time()
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[RFIDNode] Publishing to sensor.reader.rfid.{self.sensor_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = ReaderMessage(
            ...
            print(f"[RFIDNode] Publishing to sensor.reader.rfid.{self.sensor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.rfid name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        sensor_id = sys.argv[1] if len(sys.argv) > 1 else "rfid_1"
        node = RFIDNode(sensor_id=sensor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[RFID] Stopped by user.")
        node.stop()