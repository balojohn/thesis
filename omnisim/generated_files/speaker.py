...
class SpeakerMessage(PubSubMessage):
    pubFreq: float
    type: str
    actuator_id: str
    soundlevelDB: float
...
class SpeakerNode(Node):
    def __init__(self, actuator_id: str = "", *args, **kwargs):
        ...
        # Create dedicated publisher for actuator.speaker
        self.publisher = self.create_publisher(
            topic=f"actuator.speaker.{self.actuator_id}",
            msg_type=SpeakerMessage,
        )

    def simulate_speaker(self, name: str):
        """
        Simulate dynamic behavior for 'Speaker' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_speaker(name)
    ...
    def start(self):
        ...
        while True:
            # DEBUG: topic_base = actuator.speaker
            ...
            # DEBUG: endpoint = Publisher, topic = actuator.led.singleled
            # DEBUG: endpoint = Publisher, topic = actuator.led.ledarray
            # DEBUG: endpoint = Publisher, topic = actuator.speaker
            ...
            print(f"[SpeakerNode] Publishing to actuator.speaker.{self.actuator_id}: {msg.model_dump()}")
            self.publisher.publish(msg)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.speaker name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actuator_id = sys.argv[1] if len(sys.argv) > 1 else "speaker_1"
        node = SpeakerNode(actuator_id=actuator_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[Speaker] Stopped by user.")
        node.stop()