...
from streamsimdsl.utils.visualizer import EnvVisualizer
from streamsimdsl.utils.geometry import node_pose_callback
# Affection handlers
from streamsimdsl.utils.affections import check_affectability
...
{% set ref_type = ref.type|lower if ref.type else "" %}
{% set ref_subtype = ref.subtype|lower if ref.subtype is defined and ref.subtype else "" %}
{% set is_rpc_sensor = ref_type in rpc_sensors or ref_subtype in rpc_sensors %}
{% if ref.class|lower == "sensor" and not is_rpc_sensor %}
from streamsimdsl.generated_files.{{ folder }}.{{ instance_name }} import {{ class_name }}Node, {{ class_name }}Message, PoseMessage
{% else %}
from streamsimdsl.generated_files.{{ folder }}.{{ instance_name }} import {{ class_name }}Node, PoseMessage
{% endif %}
{% endfor %}
...
{%- macro search_rpc_sensors(obj) -%}
    ...
    {%- if ssub in rpc_sensors or stype in rpc_sensors -%}
        ...
        -%}
from streamsimdsl.generated_files.{{ folder }}.{{ instance_name }} import {{ RpcClass }}
    {% endif -%}
    ...
    "properties": {
                ...
                "{{ attr }}":
                    ...
                    {%- elif val.__class__.__name__ == "list"
                        and val
                        and val[0].__class__.__name__ == "AffectEntry" -%}
                    [
                        {%- for entry in val -%}
                            {
                    "id": "{{ entry.id }}"
                    {%- if entry.dispersion is not none -%},
                        "dispersion": {
                            "type": "{{ entry.dispersion.__class__.__name__ }}"
                            {%- for k,v in entry.dispersion.__dict__.items()
                                if k not in ["_tx_model","_tx_position","_tx_position_end","parent","ref"]
                                and v is not none -%},
                                "{{ k }}": {{ v|tojson }}
                            {%- endfor -%}
...
class {{ environment.name }}Node(Node):
    def __init__(self, env_name: str, *args, **kwargs):
        ...
        {% set rpc_sensors = ["camera", "rfid", "microphone"] %}
        {% set sensor_type = type|lower if type else "" %}
        {% set sensor_subtype = subtype|lower if subtype is defined and subtype else "" %}
        {% set is_rpc_sensor = sensor_type in rpc_sensors or sensor_subtype in rpc_sensors %}
        {% if cls == "sensor" and not is_rpc_sensor %}
