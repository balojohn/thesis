...
class SpeakerNode(Node):
    def __init__(self, actuator_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actuator.speaker.{self.actuator_id}.pose",
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[SpeakerNode] Publishing to actuator.speaker.{self.actuator_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = SpeakerMessage(
                pubFreq=self.pub_freq,
                actuator_id=self.actuator_id,
                type="SpeakerData",
                soundlevelDB=float(self.get_property_value("soundlevelDB")),
            )
            print(f"[SpeakerNode] Publishing to actuator.speaker.{self.actuator_id}: {msg_data.model_dump()}")
