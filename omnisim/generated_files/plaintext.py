...
class PlainTextNode(Node):
    def __init__(self, actor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actor.text.plaintext.{self.actor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actor.text.plaintext
        self.data_publisher = self.create_publisher(
            topic=f"actor.text.plaintext.{self.actor_id}",
    ...
    def simulate_plaintext(self, name: str):
        """
        Simulate dynamic behavior for 'PlainText' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_plaintext(name)
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[PlainTextNode] Publishing to actor.text.plaintext.{self.actor_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = TextMessage(
                pubFreq=self.pub_freq,
                actor_id=self.actor_id,
                type="TextData",
                message=str(self.get_property_value("message")),
            )
            print(f"[PlainTextNode] Publishing to actor.text.plaintext.{self.actor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.plaintext name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actor_id = sys.argv[1] if len(sys.argv) > 1 else "plaintext_1"
        node = PlainTextNode(actor_id=actor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[PlainText] Stopped by user.")
        node.stop()