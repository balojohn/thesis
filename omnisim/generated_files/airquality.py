...
class AirQualityNode(Node):
    def __init__(self, sensor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"sensor.envsensor.airquality.{self.sensor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for sensor.envsensor.airquality
        self.data_publisher = self.create_publisher(
            topic=f"sensor.envsensor.airquality.{self.sensor_id}",
    ...
    def simulate_airquality(self, name: str):
        """
        Simulate dynamic behavior for 'AirQuality' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_airquality(name)
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[AirQualityNode] Publishing to sensor.envsensor.airquality.{self.sensor_id}.pose: {msg_pose.model_dump()}")
            ...
            print(f"[AirQualityNode] Publishing to sensor.envsensor.airquality.{self.sensor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.airquality name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        sensor_id = sys.argv[1] if len(sys.argv) > 1 else "airquality_1"
        node = AirQualityNode(sensor_id=sensor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[AirQuality] Stopped by user.")
        node.stop()