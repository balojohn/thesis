...
class RelayMessage(PubSubMessage):
    pubFreq: float
    type: str
    actuator_id: str
    state: str
    allowed_states: float
...
class RelayNode(Node):
    def __init__(self, actuator_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actuator.relay.{self.actuator_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actuator.relay
        self.data_publisher = self.create_publisher(
            topic=f"actuator.relay.{self.actuator_id}",
            msg_type=RelayMessage,
        )
    ...
    def simulate_relay(self, name: str):
        """
        Simulate dynamic behavior for 'Relay' actor.
        """
        t = time.time()
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[RelayNode] Publishing to actuator.relay.{self.actuator_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = RelayMessage(
                pubFreq=self.pub_freq,
                actuator_id=self.actuator_id,
                type="RelayData",
                state=str(self.get_property_value("state")),
                allowed_states=float(self.get_property_value("allowed_states")),
            )
            print(f"[RelayNode] Publishing to actuator.relay.{self.actuator_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.relay name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actuator_id = sys.argv[1] if len(sys.argv) > 1 else "relay_1"
        node = RelayNode(actuator_id=actuator_id)
        node.start()
