...
class RFIDTagNode(Node):
    def __init__(self, actor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actor.text.rfidtag.{self.actor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actor.text.rfidtag
        self.data_publisher = self.create_publisher(
            topic=f"actor.text.rfidtag.{self.actor_id}",
    ...
    def simulate_rfidtag(self, name: str):
        """
        Simulate dynamic behavior for 'RFIDTag' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_rfidtag(name)
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[RFIDTagNode] Publishing to actor.text.rfidtag.{self.actor_id}.pose: {msg_pose.model_dump()}")
            ...
            print(f"[RFIDTagNode] Publishing to actor.text.rfidtag.{self.actor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.rfidtag name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actor_id = sys.argv[1] if len(sys.argv) > 1 else "rfidtag_1"
        node = RFIDTagNode(actor_id=actor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[RFIDTag] Stopped by user.")
        node.stop()