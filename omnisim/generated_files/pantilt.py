...
class PanTiltMessage(PubSubMessage):
    pubFreq: float
    actuator_id: str
    panAngle: float
    tiltAngle: float
...
class PanTiltNode(Node):
    def __init__(self, actuator_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actuator.pantilt.{self.actuator_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actuator.pantilt
        self.data_publisher = self.create_publisher(
            topic=f"actuator.pantilt.{self.actuator_id}",
            msg_type=PanTiltMessage,
        )
    ...
    def simulate_pantilt(self, name: str):
        """
        Simulate dynamic behavior for 'PanTilt' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_pantilt(name)
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[PanTiltNode] Publishing to actuator.pantilt.{self.actuator_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = PanTiltMessage(
                pubFreq=self.pub_freq,
                actuator_id=self.actuator_id,
                type="PanTiltData",
                panAngle=float(self.get_property_value("panAngle")),
                tiltAngle=float(self.get_property_value("tiltAngle")),
            )
            print(f"[PanTiltNode] Publishing to actuator.pantilt.{self.actuator_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.pantilt name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actuator_id = sys.argv[1] if len(sys.argv) > 1 else "pantilt_1"
        node = PanTiltNode(actuator_id=actuator_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[PanTilt] Stopped by user.")
        node.stop()