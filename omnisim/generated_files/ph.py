...
class PHNode(Node):
    def __init__(self, sensor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"sensor.envsensor.ph.{self.sensor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for sensor.envsensor.ph
        self.data_publisher = self.create_publisher(
            topic=f"sensor.envsensor.ph.{self.sensor_id}",
    ...
    def simulate_ph(self, name: str):
        """
        Simulate dynamic behavior for 'PH' actor.
        """
        t = time.time()
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[PHNode] Publishing to sensor.envsensor.ph.{self.sensor_id}.pose: {msg_pose.model_dump()}")
            ...
            print(f"[PHNode] Publishing to sensor.envsensor.ph.{self.sensor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.ph name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        sensor_id = sys.argv[1] if len(sys.argv) > 1 else "ph_1"
        node = PHNode(sensor_id=sensor_id)
        node.start()
