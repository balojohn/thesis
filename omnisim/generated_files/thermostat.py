...
class ThermostatNode(Node):
    def __init__(self, actuator_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actuator.envdevice.thermostat.{self.actuator_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actuator.envdevice.thermostat
        self.data_publisher = self.create_publisher(
            topic=f"actuator.envdevice.thermostat.{self.actuator_id}",
    ...
    def simulate_thermostat(self, name: str):
        """
        Simulate dynamic behavior for 'Thermostat' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_thermostat(name)
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[ThermostatNode] Publishing to actuator.envdevice.thermostat.{self.actuator_id}.pose: {msg_pose.model_dump()}")
            ...
            print(f"[ThermostatNode] Publishing to actuator.envdevice.thermostat.{self.actuator_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.thermostat name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actuator_id = sys.argv[1] if len(sys.argv) > 1 else "thermostat_1"
        node = ThermostatNode(actuator_id=actuator_id)
        node.start()
