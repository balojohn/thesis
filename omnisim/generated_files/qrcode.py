...
class QRcodeNode(Node):
    def __init__(self, actor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actor.text.qrcode.{self.actor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actor.text.qrcode
        self.data_publisher = self.create_publisher(
            topic=f"actor.text.qrcode.{self.actor_id}",
    ...
    def simulate_qrcode(self, name: str):
        """
        Simulate dynamic behavior for 'QRcode' actor.
        """
        t = time.time()
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[QRcodeNode] Publishing to actor.text.qrcode.{self.actor_id}.pose: {msg_pose.model_dump()}")
            ...
            print(f"[QRcodeNode] Publishing to actor.text.qrcode.{self.actor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.qrcode name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actor_id = sys.argv[1] if len(sys.argv) > 1 else "qrcode_1"
        node = QRcodeNode(actor_id=actor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[QRcode] Stopped by user.")
        node.stop()