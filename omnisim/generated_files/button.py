...
class ButtonNode(Node):
    def __init__(self, actuator_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actuator.button.singlebutton.{self.actuator_id}.pose",
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[ButtonNode] Publishing to actuator.button.singlebutton.{self.actuator_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = ButtonMessage(
                pubFreq=self.pub_freq,
                actuator_id=self.actuator_id,
                type="ButtonData",
                state=int(self.get_property_value("state")),
            )
            print(f"[ButtonNode] Publishing to actuator.button.singlebutton.{self.actuator_id}: {msg_data.model_dump()}")
