...
class MicrophoneNode(Node):
    def __init__(self, sensor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"sensor.microphone.{self.sensor_id}.pose",
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[MicrophoneNode] Publishing to sensor.microphone.{self.sensor_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = MicrophoneMessage(
                pubFreq=self.pub_freq,
                sensor_id=self.sensor_id,
                type="MicrophoneData",
                soundlevelDB=float(self.get_property_value("soundlevelDB")),
                mode=str(self.get_property_value("mode")),
                blocked=int(self.get_property_value("blocked")),
            )
            print(f"[MicrophoneNode] Publishing to sensor.microphone.{self.sensor_id}: {msg_data.model_dump()}")
