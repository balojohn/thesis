...
class ColorNode(Node):
    def __init__(self, actor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actor.color.{self.actor_id}.pose",
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[ColorNode] Publishing to actor.color.{self.actor_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = ColorMessage(
                pubFreq=self.pub_freq,
                actor_id=self.actor_id,
                type="ColorData",
                r=int(self.get_property_value("r")),
                g=int(self.get_property_value("g")),
                b=int(self.get_property_value("b")),
            )
            print(f"[ColorNode] Publishing to actor.color.{self.actor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.color name
