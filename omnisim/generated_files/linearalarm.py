...
class LinearAlarmNode(Node):
    def __init__(self, sensor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        super().__init__(
            node_name="linearalarm",
            connection_params=ConnectionParameters(),
            *args, **kwargs
        )

        self.pose_publisher = self.create_publisher(
            topic=f"sensor.alarm.linearalarm.{self.sensor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for sensor.alarm.linearalarm
        self.data_publisher = self.create_publisher(
            topic=f"sensor.alarm.linearalarm.{self.sensor_id}",
    ...
    def simulate_linearalarm(self, name: str):
        """
        Simulate dynamic behavior for 'LinearAlarm' actor.
        """
        t = time.time()
    ...
    def get_property_value(self, name):
        if name in SIMULATED_PROPS:
            val = self.simulate_linearalarm(name)
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[LinearAlarmNode] Publishing to sensor.alarm.linearalarm.{self.sensor_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = AlarmMessage(
            ...
            print(f"[LinearAlarmNode] Publishing to sensor.alarm.linearalarm.{self.sensor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.linearalarm name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        sensor_id = sys.argv[1] if len(sys.argv) > 1 else "linearalarm_1"
        node = LinearAlarmNode(sensor_id=sensor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[LinearAlarm] Stopped by user.")
        node.stop()