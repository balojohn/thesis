...
class LightMessage(PubSubMessage):
    pubFreq: float
    type: str
    sensor_id: str
    luminosity: float
...
class LightNode(Node):
    def __init__(self, sensor_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"sensor.light.{self.sensor_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for sensor.light
        self.data_publisher = self.create_publisher(
            topic=f"sensor.light.{self.sensor_id}",
            msg_type=LightMessage,
        )
    ...
    def simulate_light(self, name: str):
        """
        Simulate dynamic behavior for 'Light' actor.
        """
        t = time.time()
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[LightNode] Publishing to sensor.light.{self.sensor_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = LightMessage(
                pubFreq=self.pub_freq,
                sensor_id=self.sensor_id,
                type="LightData",
                luminosity=float(self.get_property_value("luminosity")),
                color=str(self.get_property_value("color")),
                minRange=float(self.get_property_value("minRange")),
                maxRange=float(self.get_property_value("maxRange")),
            )
            print(f"[LightNode] Publishing to sensor.light.{self.sensor_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.light name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        sensor_id = sys.argv[1] if len(sys.argv) > 1 else "light_1"
        node = LightNode(sensor_id=sensor_id)
        node.start()
    except KeyboardInterrupt:
        print(f"\n[Light] Stopped by user.")
        node.stop()