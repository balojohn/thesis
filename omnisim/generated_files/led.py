...
class LedMessage(PubSubMessage):
    pubFreq: float
    type: str
    actuator_id: str
    color: int
...
class LedNode(Node):
    def __init__(self, actuator_id: str = "", initial_pose: dict | None = None, *args, **kwargs):
        ...
        self.pose_publisher = self.create_publisher(
            topic=f"actuator.led.singleled.{self.actuator_id}.pose",
            msg_type=PoseMessage
        )
            
        # Create dedicated publisher for actuator.led.singleled
        self.data_publisher = self.create_publisher(
            topic=f"actuator.led.singleled.{self.actuator_id}",
            msg_type=LedMessage,
        )
    ...
    def simulate_led(self, name: str):
        """
        Simulate dynamic behavior for 'Led' actor.
        """
        t = time.time()
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"[LedNode] Publishing to actuator.led.singleled.{self.actuator_id}.pose: {msg_pose.model_dump()}")
            self.pose_publisher.publish(msg_pose)
            msg_data = LedMessage(
                pubFreq=self.pub_freq,
                actuator_id=self.actuator_id,
                type="LedData",
                color=int(self.get_property_value("color")),
            )
            print(f"[LedNode] Publishing to actuator.led.singleled.{self.actuator_id}: {msg_data.model_dump()}")
            self.data_publisher.publish(msg_data)
            rate.sleep()

# Run it from C:\thesis\ by: python -m omnisim.generated_files.led name
if __name__ == '__main__':
    ...
    try:
        try:
            ...
            exit(1)
        actuator_id = sys.argv[1] if len(sys.argv) > 1 else "led_1"
        node = LedNode(actuator_id=actuator_id)
        node.start()
